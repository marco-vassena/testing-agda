...
The data type is indexed over a \texttt{BListTree} which keeps track of the types of the quantified variables and whose goal will be explained in the following.
...
⟦ Property P ⟧ = Dec P
\end{code}
For each quantified variable, denoted by existential and universal quantification  constructors, we require an argument of that type. The rest of the signature is 
...
The module \texttt{Test.Result} contains a variant of result, which is used to give feedback to the user. It is obtained by post-processing the internal result.
...
Therefore in the cases \texttt{NotFor x r} and \texttt{Exists x r}, x is hidden and substituted with its type. 
